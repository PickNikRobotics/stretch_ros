...
def generate_launch_description():
    ...
    move_group_capabilities = {'capabilities': """move_group/ExecuteTaskSolutionCapability move_group/TfPublisher"""}
    ...
    # RViz
    rviz_config_file = get_package_share_directory('pick_place_demo') + "/rviz/mtc.rviz"
    ...
    fake_joint_driver_node = Node(
        ...
        parameters=[robot_description, os.path.join(get_package_share_directory("stretch_moveit_config"), "config",
    ...
    load_joint_state_controller = ExecuteProcess(cmd=['ros2 control load_start_controller joint_state_controller'],
    ...
    for controller in ['stretch_arm_controller', 'gripper_controller']:
        load_controllers += [
            ExecuteProcess(cmd=['ros2 control load_configure_controller', controller], shell=True, output='screen',
                           on_exit=[
                               ExecuteProcess(cmd=['ros2 control switch_controllers --start-controllers', controller],
                                              shell=True, output='screen')])]

    return LaunchDescription(
        [rviz_node, static_tf, robot_state_publisher, run_move_group_node, fake_joint_driver_node] + load_controllers)
